...
#include "BoxCylinderCollisionResolveFunction.h"

void BoxCylinderCollisionResolveFunction::Resolve(COL_HIT_DATA data)
{
	...
	auto [boxCol, cylinderCol] = ConvertCollider<BoxCollider, CylinderCollider>
		(data.targetCollider, data.selfCollider, COLLISION_SHAPE_TYPE::BOX, COLLISION_SHAPE_TYPE::CYLINDER);

	if (boxCol->IsStatic())
	{
		ResolveStaticBox(data);
	}
	else if (cylinderCol->IsStatic())
	{
		ResolveStaticCylinder(data);
	}
}

void BoxCylinderCollisionResolveFunction::ResolveStaticBox(COL_HIT_DATA data)
{
...
void BoxCylinderCollisionResolveFunction::ResolveStaticCylinder(COL_HIT_DATA data)
{
}
